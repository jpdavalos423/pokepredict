...
echo "[phase1] Seeding cards into $TABLE_CARDS"
TABLE_CARDS="$TABLE_CARDS" pnpm generate:data
...
if [[ "$RUN_PIPELINE" == "true" ]]; then
  ...
  echo "[phase1] Triggering manual execution ($RUN_ID)"
  EXEC_ARN="$(aws stepfunctions start-execution \
    --state-machine-arn "$INGESTION_ARN" \
    --input "{\"source\":\"fixture\",\"mode\":\"manual\",\"runId\":\"$RUN_ID\",\"asOf\":\"$AS_OF\"}" \
